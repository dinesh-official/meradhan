...
import AuthenticationLogsCardHeaderFilters from "../authentication/_components/AuthenticationLogsCardHeaderFilters";
...
function AuthenticationActivityLogsView() {
  ...
  const fetchLogsQuery = useQuery({
    queryKey: ["web-activity-logs", filters],
    queryFn: async () => {
      const response = await logsApi.getGroupedWebAuditLogs({
        page: filters.page || 1,
  ...
  return (
    <Card>
      <AuthenticationLogsCardHeaderFilters
        ...
        description="tracking of user actions and system events"
          ...
          <div className="flex flex-col justify-center items-center w-full h-40 text-gray-500">
            ...
            <p className="text-sm text-center">
              {filters.search || filters.userId
                ...
                : "No activity activity recorded yet"}
            </p>
          </div>
        ) : (
