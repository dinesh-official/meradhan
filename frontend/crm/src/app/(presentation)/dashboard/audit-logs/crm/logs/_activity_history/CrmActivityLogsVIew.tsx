...
function CrmActivityLogsVIew() {
  ...
  const activityData = useQuery({
    ...
    queryFn: async () => {
      const response = await apiCaller.getActivityLogsCrm({
        page,
        pageSize,
  ...
  const handleExport = () => {
    ...
    filteredData.forEach((item) => {
      ...
      const browserStr = item.browserName?.trim() || "N/A";
      const osStr = item.operatingSystem?.trim() || "N/A";
