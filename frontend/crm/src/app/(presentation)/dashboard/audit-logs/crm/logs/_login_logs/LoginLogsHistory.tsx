...
export function LoginLogsHistory() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["login-logs", page, pageSize, startDate, endDate],
    queryFn: async () => {
      const response = await apiGet.getLoginLogsCrm({
        page,
        pageSize,
  ...
  const filteredData = data?.data.filter((log) => {
    const matchesSearch =
      ...
      log.ipAddress.includes(debouncedSearch);
        ...
        <div className="space-y-4">
          ...
          <div className="text-xs text-muted-foreground">
            {isLoading ? (
              ...
              <>
                Showing {filteredData?.length || 0} of {data?.meta.total || 0}{" "}
                login attempts
              </>
            )}
          </div>
