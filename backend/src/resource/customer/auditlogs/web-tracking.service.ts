...
import { WebTrackingRepo } from "./web-tracking.repo";
...
/**
 * Service for Web tracking audit logs (customer website tracking)
 */
export class WebTrackingService {
  private auditLogsRepo: WebTrackingRepo;
  private auth_list = [
    "login",
    "logout",
    "auto_logout",
  ...
  constructor() {
    this.auditLogsRepo = new WebTrackingRepo();
  }
  ...
  async createTracking(payload: TrackingPayload) {
    ...
    const auditLogData: DataBaseSchema.WebAuditLogsCreateInput = {
      type: payload.type,
    ...
    const sessions = await Promise.all(
      groups.map(async (g) => {
        ...
        const user = latest?.userId
          ? await this.auditLogsRepo.findCustomerUser(latest.userId)
          : null;
